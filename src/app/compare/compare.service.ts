...
export class CompareService {
  ...
  onStoreData(data: CompareData) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http.post('https://qyb0aha5yj.execute-api.us-east-2.amazonaws.com/dev/compare-yourself', data, {
  ...
  onRetrieveData(all = true) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http.get('https://qyb0aha5yj.execute-api.us-east-2.amazonaws.com/dev/compare-yourself' + urlParam + queryParam, {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken()})
      })
        .map(
